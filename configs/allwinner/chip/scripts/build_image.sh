...
if [ -z "$CHIP_IMAGE" ]; then
    echo "Please set CHIP_IMAGE to the path to the output image."
    exit 1
fi
...
if [[ "$CHIP_IMAGE" != /* ]]; then
    ...
    CHIP_IMAGE=$SKIFF_ROOT_DIR/$CHIP_IMAGE
fi

echo "Allocating sparse image..."
fallocate -l 1G $CHIP_IMAGE

echo "Setting up loopback device..."
export CHIP_SD=$(losetup --show -fP $CHIP_IMAGE)
function cleanup {
  echo "Removing loopback device..." || true
  sync || true
  losetup -d $CHIP_SD || true
}
trap cleanup EXIT

if [ -z "${CHIP_SD}" ] || [ ! -b ${CHIP_SD} ]; then
...
echo "Using loopback device at ${CHIP_SD}"
